...
function start(){
    ...
    setTimeout(function(){title.className += " invisible";title.innerHTML = 3;ok = true;state.current = state.ingame;animation_game();document.getElementById("btnPause").classList.remove("invisible");},4000);
}
...
function character(){    
    ...
    if(xObstacles>-127 && xObstacles<209 && speed>64.6){
        beforeGameOver();
        state.current = state.gameOver;
    }
    if(xObstacles>153 && xObstacles<205 && speed<44.6){
        beforeGameOver();
        state.current = state.gameOver;
    }
}
